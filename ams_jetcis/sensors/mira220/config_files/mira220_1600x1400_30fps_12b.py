# Version 12 (22/09/2021)
# VDD25 implementation
import time

def getSensorInfo(imager):
    '''Sensor information related to this config file.
    '''
    controlParams = {
        "width" : 1600,
        "height" : 1400,
...
def resetSensor(imager):
    '''Set the output power supplies of the PMIC going to the sensor. Put some LEDs on.
    '''
    # Set I2C address
    ...
    imager.type(0)
    # Turn off LED driver
    if imager.write(0x10, 0x08) == 1:
        # JTAG to GND. Interposer has no led driver
        imager.setSensorI2C(0x0a)
        imager.type(0)
        imager.write(11, 0b11011111)
        imager.write(15, 0b00000000)
        imager.write(6, 1)
...
# Mira220 global variables
len_eoi = 0
f_clk_in = 38.4e6
otp_gain_trims = []

def calc_glob(imager):
    '''Calculate the FOT/GLOB time.
    '''
    global len_eoi
...
def get_sensor_revision(imager):
    '''Return the revision of the sensor programmed in OTP
    '''
    # Set I2C address
    imager.setSensorI2C(0x54)
    imager.type(1)
...
def trim_restore(imager, rev):
    ...
    if (otp_read(imager, 0x1d) != 0xff):
        ...
        if rev == 2:
            imager.write(0x403e, otp_read(imager, 0x0d))
    # OTP power off
    imager.write(0x0080, 0x08)
...
def initSensor(imager):
    ...
    ## Get sensor revision
    rev = get_sensor_revision(imager)
    
    ## Disable internal LDOs connected to VDD25
    if rev == 2:
        imager.write(0x401e, 0x02)
        imager.write(0x4038, 0x3b)
