...
#        self.unlockI2C()
    ...
    def clearArgus(self):
        ...
        sp = os.popen("echo {} | sudo -S lsof -w {}".format(self.rootPW, self.fname))
        result = sp.read()
        sp.close()
        print(result)
        for line in result.splitlines():
            ...
            if (ll[0] == "COMMAND"):
                ...
                result = os.popen("echo {} | sudo -S /usr/bin/gdb -x /tmp/gdb.cmd -p {}".format(self.rootPW, pid))
                print(result)
                print(result.read())
                print("done")
        time.sleep(0.5)
    ...
    def saveTiff(self, fname, count=1):
        ...
        cnt = count
        sp = subprocess.Popen(["/usr/bin/v4l2-ctl", "-d", self.fname, self.format, "--set-ctrl", "bypass_mode=0", "--stream-mmap", "--stream-count={}".format(cnt), "--stream-to=/tmp/record.raw"], stdin=subprocess.PIPE)
        # (result, err) = sp.communicate(self.rootPW.encode())
...
#            try:
        ...
        img=img[::,0:self.h,0:self.w]*2**(16-self.bpp) #scale to 16 bit tiff
...
#              pass # not enough image data
    ...
    def saveRaw(self, fname, count=1):
        ...
        if (count == 1):
            ...
            cnt = count
        sp = subprocess.Popen(["/usr/bin/v4l2-ctl", "-d", self.fname, self.format, "--set-ctrl", "bypass_mode=0", "--stream-mmap", "--stream-count={}".format(cnt), "--stream-to=/tmp/record.raw"])
#        sp.terminate()
        sp.wait()
        #time.sleep(2)
        # finally copy raw from RAM to final location. remove stuffing data and extract single frames
        for fno in range(0, cnt):
            ...
            if (self.bpp > 8):
                ...
                bpp = 1
                dt = np.uint8
            sp = subprocess.Popen(["/bin/dd", "if=/tmp/record.raw", "of=/tmp/record_tmp.raw", "bs={}".format(w*bpp), "count={}".format(h), "skip={}".format(h*fno)])
            sp.wait()
            # cut out data
...
#                self.pr("cutting no={} to={}".format(fno, f))
    ...
    def initISP_test(self, w, h, fps, id, mode):
        Gst.init(None)
        self.gstpipe = Gst.Pipeline.new("Capture")
        self.src = Gst.ElementFactory.make("nvarguscamerasrc", "src")
        self.src.set_property("sensor-id", id)
